...
3. `occupants` 必要时清占用（脚本在 `scripts/`）。独占步骤还会按进程名结束 Unity 编辑器族（不依赖命令行是否含工程路径）。`svn-update` / `svn-revert` 另按本次写集把具体文件交给 Restart Manager；步骤可用 `with.releaseOccupants: false` 关掉自动杀进程。
