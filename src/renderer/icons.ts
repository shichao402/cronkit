// Inline SVG keeps the renderer dependency-free and lets icons inherit
// `currentColor`, so they follow the theme without a second colour system.

const OUTLINE: Record<string, string> = {
  gauge: '<circle cx="12" cy="12" r="8.5"/><path d="M12 12l4.2-4.2"/>',
  sliders: '<path d="M4 7h16M4 12h16M4 17h16"/><circle cx="15.5" cy="7" r="2"/><circle cx="8.5" cy="12" r="2"/><circle cx="17" cy="17" r="2"/>',
  gear: '<circle cx="12" cy="12" r="3.2"/><path d="M12 2.6v3M12 18.4v3M4.4 7.2l2.6 1.5M17 15.3l2.6 1.5M4.4 16.8l2.6-1.5M17 8.7l2.6-1.5"/>',
  folder: '<path d="M3 7.5A1.5 1.5 0 0 1 4.5 6h3.8l1.9 2.4h6.3A1.5 1.5 0 0 1 18 9.9v7.6A1.5 1.5 0 0 1 16.5 19h-12A1.5 1.5 0 0 1 3 17.5z"/>',
  ...
  external: '<path d="M14 4.5h5.5V10M19.5 4.5 12 12"/><path d="M18 13.5v4.6A1.4 1.4 0 0 1 16.6 19.5H5.9A1.4 1.4 0 0 1 4.5 18.1V7.4A1.4 1.4 0 0 1 5.9 6h4.6"/>',
  ...
  inbox: '<path d="M4 13.5h4l1.4 2.8h5.2L16 13.5h4"/><path d="M4 13.5 6.6 5.5h10.8L20 13.5v4.6A1.4 1.4 0 0 1 18.6 19.5H5.4A1.4 1.4 0 0 1 4 18.1z"/>',
  calendar: '<rect x="3.5" y="5.8" width="17" height="14.7" rx="2"/><path d="M3.5 10.4h17M8 3.5v4.2M16 3.5v4.2"/>',
  ...
  terminal: '<rect x="3.5" y="4.8" width="17" height="14.4" rx="2"/><path d="M7.5 10 10 12.4l-2.5 2.4M12.5 15h4.2"/>',
  spinner: '<path d="M12 3.5a8.5 8.5 0 1 1-6 2.5" />',
};

const FILLED = new Set(["play", "stop", "dot"]);

export type IconName = keyof typeof OUTLINE | "dot";

export function icon(name: IconName, className = ""): string {
  if (name === "dot") {
    return `<svg class="icon ${className}" viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill="currentColor"><circle cx="12" cy="12" r="5"/></svg>`;
  }
  const filled = FILLED.has(name);
  return `<svg class="icon ${className}" viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill="${
    filled ? "currentColor" : "none"
  }" stroke="${filled ? "none" : "currentColor"}" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${
    OUTLINE[name]
  }</svg>`;
}
